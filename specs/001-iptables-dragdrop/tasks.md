...
- [x] T001 Initialize Gradle Java application in the repository root with `build.gradle`, `settings.gradle`, and Gradle wrapper files
- [x] T002 Create source directories `src/main/java/app/`, `src/main/resources/`, `src/test/java/app/`
- [x] T003 Add JavaFX and JUnit 5 dependencies to `build.gradle` and configure the `application` plugin
- [x] T004 [P] Create initial application entry point in `src/main/java/app/Main.java`
- [x] T005 [P] Create base UI package and empty JavaFX scene files in `src/main/java/app/ui/`
...
- [x] T006 Create `Rule.java` in `src/main/java/app/model/Rule.java` with fields for origem, destino, protocolo, portaOrigem, portaDestino, interfaceEntrada, interfaceSaida, acao, comentario, and posicao
- [x] T007 Create `RuleSet.java` in `src/main/java/app/model/RuleSet.java` to manage an ordered list of rules and filtered preview state
- [x] T008 Implement `RuleValidator.java` in `src/main/java/app/service/RuleValidator.java` with IP/CIDR validation, port range validation, protocol constraints, and duplicate rule detection
- [x] T009 Implement `IptablesScriptGenerator.java` in `src/main/java/app/service/IptablesScriptGenerator.java` to translate an ordered `RuleSet` into plain text `iptables` filter commands
- [x] T010 [P] Create unit tests for rule modeling and validation in `src/test/java/app/model/RuleTest.java` and `src/test/java/app/model/RuleSetTest.java`
- [x] T011 [P] Create unit tests for script generation in `src/test/java/app/service/IptablesScriptGeneratorTest.java`
- [x] T012 [P] Create unit tests for validation rules in `src/test/java/app/service/RuleValidatorTest.java`
...
- [x] T013 [US1] Implement `RuleBuilderView.java` in `src/main/java/app/ui/RuleBuilderView.java` to expose rule component fields and drag-and-drop creation interactions
